"""
Python library for CCI correlation analysis.
...
from core.py import FFT, reconstruct #(Update function names)
...
def dyn_factor(image,image_ref,method = 'scalarproduct', crop = 0, plot = False, print_out = False):
    ...
    if method == 'scalarproduct':
        ...
        image_ref = xp.array(image_ref)
        factor = xp.sum(image[crop_s]*image_ref[crop_s])/xp.sum(image_ref[crop_s]*image_ref[crop_s])
        offset = 0

        if GPU is True:
            factor = xp.asnumpy(factor)
    ...
    #Not optimized for gpu supportyet
        ...
        if plot == True:
            ...
            ax.plot(xdata,lin_func(xdata,*popt),'r-')
...
def filter_reference(holo,mask,settings):
    ...
    #Transform to xp array
    holo = xp.array(holo)
    mask = xp.array(mask)
    
    #Transform to Patterson map
    tmp_array = xp.array(reconstruct(holo))
...
def seg_statistics(holo, mask, NrStd = 1):
    ...
    Statistics_mask = (xp.abs(holo) >= MEAN + NrStd*STD)
...
def create_ring_mask(shape,center, radi):
    '''
    Creates mask of ring sements of given radius
...
def correlate_holograms(diff1, diff2, sum1, sum2, Statistics1, Statistics2):
    ...
    sf = xp.sqrt(xp.sum(S1 * S1)*xp.sum(S2 * S2))
...
def correlation_map(diff_holo_norm, statistics_mask):
    ...
    #Convert to cupy
    diff_holo_norm = xp.array(diff_holo_norm) 
    statistics_mask = xp.array(statistics_mask)
                             
    #predefine array
    corr_map = xp.eye(diff_holo_norm.shape[0])
    ...
    for ii in tqdm(range(diff_holo_norm.shape[0])):
        ...
        mask = xp.logical_or(statistics_1,statistics_mask[ii+1:])
        ...
        sf = xp.sqrt(xp.sum(holo_1 * holo_1, axis = (1,2))*xp.sum(holo_2 * holo_2, axis = (1,2)))
...
def correlation_map_fast(in_array):
    ...
    #predefine array
    corr_map_pearson = xp.zeros((in_array.shape[0],in_array.shape[0]))
    corr_map_nonorm = xp.zeros((in_array.shape[0],in_array.shape[0]))
    corr_map_sutton = xp.zeros((in_array.shape[0],in_array.shape[0]))
